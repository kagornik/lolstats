...
import {grandmaster} from '../assets';
import styled from 'styled-components';
import Icon from 'react-native-vector-icons/MaterialIcons';
import {connect} from 'react-redux';

const mapStateToProps = (state) => ({
  ranksTFT: state.ranksTFT,
});

const TopRankTFT = (props) => {
  return (
    <ImageBackground
      source={Background}
      style={{
      ...
      <Container>
        ...
        <Rank>
          <Image
            ...
            source={grandmaster}
          />
          <TextMenu>Rank TFT</TextMenu>
        </Rank>
        <ScrollView style={{width: '100%'}}>
          {props.ranksTFT.map((element, key) => (
            <Summoner key={key}>
              <View
                style={{
...
export default connect(mapStateToProps, null)(TopRankTFT);
